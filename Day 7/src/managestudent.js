...
const ManageStudents = () => {
  ...
  const handleAddStudent = () => {
    ...
    if (editStudentIndex !== null) {
      ...
      const studentsToAdd = []; // Array to hold the new students
  
      // Extract each student from the newStudent object
      const { id, rollNo, name, email, password } = newStudent;
      const periods = columnNames.map((columnName) => ({
        period: columnName,
        present: true,
      }));
  
      // Split rollNo, name, email, and password into arrays
      const rollNos = rollNo.split(',');
      const names = name.split(',');
      const emails = email.split(',');
      const passwords = password.split(',');
  
      // Check if the number of entries for each field is the same
      if (rollNos.length !== names.length || rollNos.length !== emails.length || rollNos.length !== passwords.length) {
        alert('Number of entries for Roll No, Name, Email, and Password must be the same!');
        return;
      }
  
      // Iterate over the arrays and create new students
      for (let i = 0; i < rollNos.length; i++) {
        const student = {
          id: generateUniqueId(),
          rollNo: rollNos[i].trim(),
          name: names[i].trim(),
          email: emails[i].trim(),
      ...
      setStudents((prevStudents) => [...prevStudents, ...studentsToAdd]);
    }
      ...
      <nav className="navbar1">
        ...
        <div className="navbar1-icons">
          {!isAddStudentFormOpen && (
            <div className="navbar1-icon" onClick={() => setIsAddStudentFormOpen(true)}>
              <FaUserPlus /> Add Student
            </div>
          )}
          {!isAddStudentFormOpen && (
            <div className="navbar1-icon" onClick={() => setIsAddStudentFormOpen(false)}>
            </div>
          )}
        </div>
      </nav>
      {isAddStudentFormOpen ? (
        ...
        <div>
          ...
          <div className="student-list">
            <h2 className='hjk'>Student List</h2>
            <table className='mst'>
              <thead>
                <tr className='tr1'>
                  ...
                  {columnNames.map((columnName) => (
                    <th className='th2' key={columnName}>{columnName.toUpperCase()}</th>
                  ))}
              ...
              <tbody>
                {searchTerm ? (
                  filteredStudents.map((student) => (
                    <tr className='tr1' key={student.id}>
                      <td className='te1'>{student.rollNo}</td>
                      <td className='te1'>{student.name}</td>
                      <td className='te1'>{student.email}</td>
                      {columnNames.map((columnName) => (
                        <td className='te1' key={columnName}>
                          <span
                            className={`status ${
